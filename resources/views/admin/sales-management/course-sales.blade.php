                                ...
                                <div class="d-flex align-items-center position-relative my-1">
                                    ...
                                    <select type="text" name="egitmen" id="trainer" data-control="select2"
                                        ...
                                        @foreach ($instructors as $instructor)
                                            <option @if ($instructor->fullname == request()->get('egitmen')) selected @endif
                                                value="{{ $instructor->fullname }}">{{ $instructor->fullname }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="d-flex align-items-center position-relative my-1">
                                    ...
                                    <select type="text" name="musteri" id="customer" data-control="select2"
                                        onchange="this.form.submit()" data-placeholder="Müşteriye Göre Ara"
                                        class="form-select form-select-solid w-250px ps-14 me-5">
                                        <option></option>
                                        @foreach ($customers as $customer)
                                            <option @if ($customer->fullname == request()->get('musteri')) selected @endif
                                                value="{{ $customer->fullname }}">{{ $customer->fullname }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @if (request()->get('kurs') || request()->get('egitmen') || request()->get('musteri'))
                                    <a href="{{ route('panel.course-sales') }}" class="btn  btn-light">
                        ...
                        <tbody class="fw-bold text-gray-600">
                            @foreach ($courseSales as $courseSale)
                                <tr class="ps-4">
                                    ...
                                    <td>{{ ucfirst($courseSale->user->fullname) }}</td>
                ...
                deleteTrainingSale(course) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "GET",
                                url: "/sistem/kurs-satislari/" + course.id + "/sil",
                                success: function() {
                                    Swal.fire({
                                        icon: "success",
                                        title: "Silindi",
