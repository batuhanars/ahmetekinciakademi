...
@section('navigation')
    <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
        ...
        <li class="breadcrumb-item text-muted">
            <a href="{{ route('panel.image-gallery.index') }}" class="text-muted text-hover-primary">Ahmet Ekinciye Ait
                Foto Galeri</a>
        </li>
        <!--end::Item-->
        <!--begin::Item-->
        ...
        <!--begin::Item-->
        <li class="breadcrumb-item text-muted">
            <a href="{{ route('panel.all-images.index') }}" class="text-muted text-hover-primary">Tüm Galeri</a>
        </li>
        <!--end::Item-->
    </ul>
@endsection
            ...
            <div class="card card-flush">
                <div class="card-header align-items-center gap-2 gap-md-5">
                    <div class="card-title">
                        ...
                        <form>
                            <div class="d-flex align-items-center">
                                ...
                                @if (Request::routeIs('panel.all-images.index'))
                                    <div class="d-flex align-items-center position-relative my-1">
                                        <span class="svg-icon svg-icon-1 position-absolute ms-4">
                                            <i class="fas fa-search"></i>
                                        </span>
                                        <select type="text" name="olusturan" id="trainer" data-control="select2"
                                            onchange="this.form.submit()" data-placeholder="Oluşturana Göre Ara"
                                            class="form-select form-select-solid w-250px ps-14 me-5">
                                            <option></option>
                                            @foreach ($users as $user)
                                                <option @if ($user->fullname == request()->get('olusturan')) selected @endif
                                                    value="{{ $user->fullname }}">{{ $user->fullname }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                @endif
                                @if (request()->get('foto') || request()->get('olusturan'))
                                    <a href="{{ Request::routeIs('panel.image-gallery.index') ? route('panel.image-gallery.index') : route('panel.all-images.index') }}"
                                        class="btn  btn-light">
                    ...
                    <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                        ...
                        <a href="{{ route('panel.image-gallery.create') }}" class="btn btn-light-primary">
                        ...
                        <tbody class="fw-bold text-gray-600">
                            @foreach ($images as $image)
                                <tr class="odd">
                                    ...
                                    <td class="text-end">
                                        <a href="{{ route('panel.image-gallery.edit', $image) }}"
                ...
                deleteImage(image) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "GET",
                                url: "/sistem/foto-galeri/" + image.id + "/sil",
                                success: function() {
                                    Swal.fire({
                                        icon: "success",
                                        title: "Silindi",
