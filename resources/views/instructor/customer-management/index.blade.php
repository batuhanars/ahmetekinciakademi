                        ...
                        <form>
                            <div class="d-flex align-items-center position-relative my-1">
                                ...
                                @if (request()->get('musteri'))
                                    <a href="{{ route('instructor.members.index') }}" class="btn  btn-light">
                        ...
                        <tbody class="fw-bold text-gray-600">
                            @foreach ($users as $user)
                                <tr class="odd">
                                    ...
                                    <td>{{ $user->phone }}</td>
                                    <td>
                                        <div class="badge badge-light-{{ $user->status == 1 ? 'success' : 'danger' }}">
                                            {{ $user->status == 1 ? 'Aktif' : 'Pasif' }}</div>
                                    </td>
                                    <td>
    ...
    <div class="modal fade" tabindex="-1" id="uploadCertificate">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('instructor.certificates.upload') }}" method="post">
                    @csrf
                ...
                deleteMember(user) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "GET",
                                url: "/panel/musteriler/" + user.id + "/sil",
                                success: function() {
                                    Swal.fire({
                                        icon: "success",
                                        title: "Silindi",
