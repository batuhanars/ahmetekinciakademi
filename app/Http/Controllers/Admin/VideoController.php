...
class VideoController extends Controller
{
    ...
    {
        $videos = Video::with("user")->orderBy("created_at", "DESC")->where("user_id", 2);
        ...
        //     ]);
        // }
        return view("admin.multi-media-management.video-gallery", compact("videos", "playlists"));
    }

    public function allVideos(Request $request)
    {
        $videos = Video::with("user")->orderBy("created_at", "DESC");
        if (request()->get("video")) {
            $videos = $videos->where("name", "LIKE", "%" . request()->get("video") . "%");
        }
        if (request()->get("egitmen")) {
            $videos = $videos->whereHas("user", function (Builder $query) {
                $query->where("fullname", request()->get("egitmen"));
            });
        }
        $videos = $videos->paginate(16);
        $playlists = DB::table("play_lists")->get();
        $users = DB::table("users")->where("membership_type", "instructor")->get();
        return view("admin.multi-media-management.video-gallery", compact("videos", "playlists", "users"));
    }
